...
function CartItems({id,name,image,category,qty,total,price}) {
  ...
  return (
    <div className='bg-slate-300 p-2 flex gap-3 rounded border border-slate-400  '>
        <div className='md:p-3 bg-white overflow-hidden rounded flex items-center '>
            <img src={image} className=' w-18 h-18 md:h-30 md:w-32 object-cover ' />
        </div>
        <div className='flex flex-col gap-1 px-2 mt-3  w-full'>
        <div className='flex md:justify-between'>
        <h3 className='text-black font-semibold capitalize text-md md:text-xl  '>{name}</h3>
        ...
        <p className='text-slate-400 font-medium text-md md:text-xl '>{category}</p>
